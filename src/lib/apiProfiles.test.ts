...
describe('mergeImportedSettings', () => {
  ...
  it('replaces the default provider list with imported OpenAI profiles when current settings are untouched', () => {
    const merged = mergeImportedSettings(DEFAULT_SETTINGS, {
      profiles: [
        {
          id: 'imported-openai',
  ...
  it('appends imported OpenAI profiles as new profiles when current settings are customized', () => {
    const current = mergeImportedSettings(DEFAULT_SETTINGS, {
      baseUrl: 'https://current.example.com/v1',
      apiKey: 'current-key',
      model: 'current-model',
...
\`\`\``)
  ...
  it('switches an OpenAI-compatible profile to a custom provider', () => {
    const provider = importCustomProviderDefinitionFromJson(JSON.stringify({
